...
export enum LinkedInRealtimeTopic {
  ...
  TabBadgeUpdateTopic = "tabBadgeUpdateTopic",
...
export const createLinkedInRealtimeTopicsSet = (
  ...
  if (!topics || topics === LINKEDIN_ALL_TOPICS) return undefined;
  const list: Array<LinkedInRealtimeTopicKey | "*"> = Array.isArray(topics)
    ? topics
    : [topics];
  if (list.includes(LINKEDIN_ALL_TOPICS)) return undefined;
  const keys = list.filter(
    (t): t is LinkedInRealtimeTopicKey => t !== LINKEDIN_ALL_TOPICS,
  );
  return new Set(keys.map((t) => LinkedInRealtimeTopic[t]));
};
